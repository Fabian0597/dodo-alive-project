...
cdef class MatrixNd:
    ...
    def __cinit__(self, msize = None, uintptr_t address=0, pyvalues = None):
        if address == 0:
            ...
            if pyvalues is not None:
                self.thisptr = new crbdl.MatrixNd(pyvalues.shape[0], pyvalues.shape[1])
                for i in range (pyvalues.shape[0]):
                    for j in range (pyvalues.shape[1]):
                        (&(self.thisptr.coeff(i,j)))[0] = pyvalues[i,j]
            else:
              if msize != None:
                xdim, ydim = msize
    ...
    def __getitem__(self, key):
        ...
        if isinstance(key, tuple):
            return self.thisptr.coeff(key[0],key[1])
        else:
            return self.thisptr.data()[key]
    ...
    @classmethod
    def fromPythonArray (cls, python_values):
        return MatrixNd (None, 0, python_values)
...
def ComputeConstraintImpulsesDirect(Model model,
    np.ndarray[double, ndim=1, mode="c"] q not None,
    np.ndarray[double, ndim=1, mode="c"] qdot_minus not None,
    ConstraintSet cs,
    np.ndarray[double, ndim=1, mode="c"] qdot_plus not None
    ):
        qdot_plus_wrap = VectorNd.fromPythonArray(qdot_plus)
        crbdl.ComputeConstraintImpulsesDirect(
            model.thisptr[0],
            NumpyToVectorNd(q),
            NumpyToVectorNd(qdot_minus),
            cs.thisptr[0],
            (<VectorNd>qdot_plus_wrap).thisptr[0]
        )
        for i in range(len(qdot_plus)):
            qdot_plus[i] = qdot_plus_wrap[i]

def CalcConstraintsJacobian(
    Model model,
    np.ndarray[double, ndim=1, mode="c"] q,
    ConstraintSet cs,
    np.ndarray[double, ndim=2, mode="c"] gOutput,
    update_kinematics = True):

    gOutput_wrap = MatrixNd.fromPythonArray(gOutput)
    crbdl.CalcConstraintsJacobian(
        model.thisptr[0],
        NumpyToVectorNd(q),
        cs.thisptr[0],
        (<MatrixNd>gOutput_wrap).thisptr[0],
        update_kinematics
    )

    for i in range (gOutput.shape[0]):
        for j in range (gOutput.shape[1]):
            gOutput[i,j] = gOutput_wrap[i,j]
